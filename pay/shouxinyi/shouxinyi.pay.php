...
$shouxinyi['data']['notifyUrl']='http'.($_SERVER["SERVER_PORT"]==443?'s':'').'://'.$_SERVER['HTTP_HOST'].substr($_SERVER["REQUEST_URI"],0,strrpos($_SERVER["REQUEST_URI"],'/')).'/shouxinyi.notify.php';
$shouxinyi['data']['callbackUrl']='http'.($_SERVER["SERVER_PORT"]==443?'s':'').'://'.$_SERVER['HTTP_HOST'].'/';
...
if($shouxinyi['decryptKey']){
    ...
    if ($shouxinyi['response']["status"] == 'REDIRECT'){
		...
		//exit($shouxinyi['response']["error"].'('.$shouxinyi['response']["cause"].')');
	}
}
